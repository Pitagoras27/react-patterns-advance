...
# How to use or understand this project

Las implementaciones de cada uno de los patrones se presenta de forma sucesiva en la capeta patterns.
El ejemplo renderizado con la opción Medium Clap presenta la funcionalidad de "Claps of Medium Site".
Para el archivo 01.js se utiliza un HOC mientras que con 02.js se implementa con un custom hook.
The file 03.js implements pattern Compound Components
The file 04.js implements pattern Reusable Styles
The file 05.js implements pattern Control Props
The file 06.js implements pattern Custom Hooks
The file 07.js implements pattern Props Collection
The file 08.js implements pattern Props Getters
The file 09.js implements pattern State Initializer
The file 10.js implements pattern State Reducers
...
```javascript
<input value={this.state.val} onChange={handleClap} />
```
  ...
  return (
    <div style={{ width: "100%" }}>
      ...
      {!!count && (
        ...
        >{`You have clapped internal count ${count} times`}</div>
      )}
      {!!state.count && (
        <div
          className={styles.info}
        >{`You have clapped user count ${state.count} times`}</div>
      )}
    </div>
  );
};
